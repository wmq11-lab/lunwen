...
系统接受Git提交产生的\allowbreak Unified Diff格式代码变更作为输入。在\allowbreak\texttt{CodeChangeAgent}的\texttt{ingest\_change}一步中，完成了多个核心子任务：Unified Diff解析、符号级变更分析（\texttt{change\_analysis}）、\allowbreak 以及变更解构图（\texttt{ChangeGraph}）的构建。\texttt{ingest\_change}步骤的输出直接写入共享状态\allowbreak\texttt{WorkflowState}中，供后续\allowbreak Agent读取。

具体而言，\texttt{ingest\_change}的内部处理流程包含以下三个核心环节：\allowbreak
首先通过\texttt{parse\_unified\_diff}解析Diff文本，提取变更文件列表（\texttt{changed\_files}）和每个文件的变更块元数据（\texttt{diff\_hunks}）；\allowbreak
然后对每个变更文件进行符号级分析，构建\texttt{ChangeRecord}列表（\texttt{change\_analysis}）；\allowbreak
最后基于变更实体构建变更解构图（\texttt{change\_graph}），建立变更符号与影响种子（\texttt{impact\_seeds}）之间的关联。
...
节点与边的权重（weight）反映关联的紧密程度，范围为$[0, 1]$。例如，\textbf{contains\_change}边的权重通常设为$1.0$，表示直接变更关系；\textbf{emits\_seed}边的权重设为$0.85$，表示符号对种子的发射强度。
...
\[
\begin{aligned}
ASU = {}& (id, type, risk\_score, priority\_score, call\_chain, \\
& downstream, upstream, edge\_types)
\end{aligned}
\]
...
语义标签在语义单元提取过程中起关键作用：从ChangeRecord的impact\_seeds和semantic\_tags出发，通过规则映射推断语义单元类型。例如，\textsf{api\_signature\_changed}和\allowbreak\textsf{dependency\_call\_changed}标签映射为api类型；\textsf{input\_validation\_added}、\allowbreak\textsf{null\_check\_added}、\allowbreak\textsf{exception\_handling\_changed}映射为exception类型；\textsf{logic\_branch\_changed}、\allowbreak\textsf{return\_value\_changed}映射为data\_processing类型。
...
\textbf{LLM推断是条件触发的可选增强}：在默认配置下（\texttt{used\_llm:\allowbreak False}），系统使用纯规则驱动的方法生成语义标签。LLM推断仅在环境变量\path{MUTIAGENT_IMPACT_LLM_REFINE}启用时触发，用于对语义标签进行精细化调整。这种设计确保了主流场景下的分析效率，同时保留了语义增强的可能性。
...
\begin{enumerate}
    \item 构建种子名称索引（\path{_seed_occurrence_index}）：遍历所有变更记录的\path{impact_seeds}，建立种子名称（去噪后）到(文件路径, 符号ID)的映射。
...
\begin{itemize}
    ...
    \item $tag\_risk(tags)$：语义标签微调因子，根据是否包含\path{null_check_added}、\allowbreak\path{input_validation_added}、\allowbreak\path{exception_handling_changed}、\allowbreak\path{logic_branch_changed}、\allowbreak\path{api_signature_changed}等标签进行累乘（上限1.12）。
\end{itemize}
...
\[
\begin{aligned}
raw\_priority = {}& risk \times change\_mult \times centrality\_mult \\
& \times \log(1 + refs) \times integration\_mult
\end{aligned}
\]
