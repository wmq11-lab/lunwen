...
\BITSetup{
  cover = {
    ...
    % 封面标题细黑体：项目根目录下的华文细黑（勿加 ./ 前缀，否则 fontspec 会误解析）
...
\usepackage{algorithm, algorithmic, amsmath, tikz, booktabs, multirow}
\usepackage{amssymb}
\usepackage{rotating}
\usepackage{pgfplots}
...
% 参考文献数据库（仅列出实际存在的文件，否则 biber 会报错）
...
% 参考文献：添加至 misc/ref.bib，正文用 \cite{…}（biblatex，勿在此处使用 \bibliography）。
% 如无特殊需要，无需改动 misc/3_reference.tex。
